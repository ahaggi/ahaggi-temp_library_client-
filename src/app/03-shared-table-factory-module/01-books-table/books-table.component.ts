...
import { deleteBookMut } from '../../util/gqlUtil/mutationsDef';
import { getBooksQry } from '../../util/gqlUtil/queriesDef';
...
export class BooksTableComponent implements OnInit {
  ...
  @Input()
  booksObsv?: Observable<any>; // row res from gqlServer
  ...
  ngOnChanges(changes: { [propKey: string]: SimpleChange }) {
    if (changes.hasOwnProperty('booksObsv') && changes['booksObsv'].currentValue != null) {
      this.booksObsv = changes['booksObsv'].currentValue;
      this.subscribeToSrc()
  ...
  ngOnInit(): void {
    ...
    if (!this.booksObsv)
      this.booksObsv = this.getAllBooksFromServerAsObsv()
